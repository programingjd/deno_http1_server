...
const listen=async(options, endpoints=[], baseUrl=toFileUrl(Deno.cwd()))=>{
  ...
  async function staticEndpoint(headers, config){
    ...
    const mimeEntries=Object.entries(mergedMimes);
    const statUrl=async(url)=>{
      try{
        return Deno.stat(url);
      }catch(_){
        return null;
      }
    }
    const cache={
      async get(key) {
        ...
        let url=new URL(sanitizePath(`${baseUrl.pathname}/${key.substring(path.length)}`),baseUrl);
        let name=url.pathname.split('/').filter(it=>it).pop();
        if(name.charAt(0)==='.') return null;
        let stat=await statUrl(url);
        if(stat?.isDirectory){
          url=new URL(sanitizePath(`${url.pathname}/index.html`),url);
          name='index.html';
          stat=await statUrl(url);
        }
        ...
        const cacheBody=cacheThreshold===null||filesize<=cacheThreshold;
        const compress=cacheBody&&mimeEntry[1].compress;
        const etag=deriveEtag(stat,filesize);
        const cacheHeaders=mergeHeaders(
          headers,
          {'content-type': mimeEntry[0],etag},
          mimeEntry[1].headers,
